	...
	<view>
		<view class="mybg">
			<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				...
				<block slot="content">医生回复</block>
			</cu-custom>

			<view class='nav-list margin-top'>
				<navigator open-type="navigate" hover-class='none':url="'/page_cure/cure_item/cure_inquiry_check_detail?inquiryid='+item.inquiryid"
				 v-for="(item, index) in answerList" :key="index">
					<view class="item myicon">
						<view class="myfont1">{{item.doctor_name}}医生的回复</view>
					</view>
				</navigator>
			</view>
...
<script>
	import {
		selectIdByAccount,doctorAnswerCheck
	...
	export default{
		onLoad() {
			...
			selectIdByAccount(Account).then((res) => {
				...
				doctorAnswerCheck(data).then((res) => {
					if (res.statusCode == "200")
					{
