	...
	<view>
		<view class="mybg">
			<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				...
				<block slot="content">上门就诊查看</block>
			</cu-custom>

			<view class='nav-list margin-top'>
				<navigator open-type="navigate" hover-class='none':url="'/page_cure/cure_item/cure_visit_check_detail?hospital_out_id='+item.hospital_out_id"
				 v-for="(item, index) in outList" :key="index">
					<!-- <text class="mytitle">{{item.doctor_name}}</text> -->
					<view class="item myicon">
						<view class="myfont1">{{item.department}}的预约</view>
					</view>
				</navigator>
			</view>
			<view style="height: 1500rpx;width: 1rpx;"></view>
		</view>
	</view>
</template>
...
<script>
	import {
		selectIdByAccount,selectAllOut
	...
	export default{
		onLoad() {
			...
			selectIdByAccount(Account).then((res) => {
				...
				let data = {villagersid: this.villagersid}
				selectAllOut(data).then((res) => {
					if (res.statusCode == "200")
					{
