...
function calLegendData(series, opts, config, chartData) {
  ...
  if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
    ...
    for (let i = 0; i < series.length; i++) {
      const item = series[i];
      const itemWidth =
        shapeWidth + shapeRight + measureText(item.legendName || 'undefined', fontSize) + opts.legend.itemGap;
    ...
    if (currentRow.length) {
      for (let i = 0; i < currentRow.length; i++) {
        ...
        for (let j = 0; j < item.length; j++) {
          const itemWidth =
            shapeWidth + shapeRight + measureText(item[j].legendName || 'undefined', fontSize) + opts.legend.itemGap;
...
function getPieTextMaxLength(series) {
  ...
  for (let i = 0; i < series.length; i++) {
    ...
    const text = item.format
      ? item.format(+item._proportion_.toFixed(2), item)
